...
| 모듈 | 설명 |
|------|------|
| **재고관리** | 제품(원자재/반제품/완제품), 창고, 재고이동, 창고간 이동, 바코드/QR 스캔, 안전재고 알림 |
| **생산관리** | BOM 관리, 생산계획, 작업지시, 생산실적(자동 재고 반영) |
| **판매관리** | 거래처, 고객, 주문, 견적(원클릭 주문전환), 출하추적, 수수료 관리 |
| **구매관리** | 발주서, 입고확인, 입고 시 자동 재고반영, 발주 상태 추적 |
| **AS관리** | AS 요청, 수리이력 추적, 보증기간 확인 |
| **회계관리** | 세금계산서, 부가세, 고정비, 손익분기점, 월별 손익, 전표, 계정과목, 원천세, 다단계 결재 |
...
| 프론트엔드 | Django Templates + Tailwind CSS(CDN) + HTMX + Alpine.js + Chart.js + FullCalendar.js |
...
u.role = 'admin'; u.name = '관리자'; u.save()
"
...
- **20개 앱**, **82+ 모델** (전체 이력 추적)
- **265+ 뷰**, **190+ 템플릿**, **270+ URL 엔드포인트**
- **~17,000줄** Python 코드 (마이그레이션 제외)
- **440+ 테스트**
- **90+ 마이그레이션**, **25개 패키지**
