...
- **[CRITICAL]** 1건: BOM 다단계(BOM of BOM) 미지원
- **[HIGH]** 4건: 입고 검수 → 불량 조정/반품 미연결, 재고 마이너스 방지 로직 부재, 조건부합격 미처리, MRP 재주문점 미지원
- **[MEDIUM]** 3건: 창고간이동 중 재고 카운팅 오류 우려, ShipmentItem 출고 예약재고 해제 시점 문제, 이동평균단가 역산 계산식 정확도
...
### [HIGH] 4. QualityInspection 조건부합격(CONDITIONAL) 미처리
...
### [HIGH] 5. 재주문점(Reorder Point) 기반 자동 발주 미지원
